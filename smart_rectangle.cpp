...
public:
    static void testValidation() {
        ...
        assert(valid.getArea() == 200);
        cout << "Valid rectangle test passed" << endl;

        // Тест с некорректными размерами (должен вывести ошибку и установить 0x0)
        SmartRectangle invalid(-5, 10);
        assert(invalid.getArea() == 0); // После коррекции должно быть 0x0
        cout << "Invalid rectangle test passed (corrected to 0x0)" << endl;

        // Тест с нулевыми размерами
        SmartRectangle zero(0, 5);
        assert(zero.getArea() == 0); // После коррекции должно быть 0x0
        cout << "Zero dimension test passed (corrected to 0x0)" << endl;

        cout << "All validation tests passed!" << endl;
    }

    static void testAreaCaching() {
        cout << "\n=== Testing Area Caching ===" << endl;
        ...
        assert(area1 == 50);
        cout << "First area calculation: " << area1 << endl;

        // Второй вызов должен вернуть кешированное значение
        double area2 = r.getArea();
        assert(area2 == 50);
        cout << "Second area calculation (cached): " << area2 << endl;

        // Проверка масштабирования
        SmartRectangle scaled = r(2.0);
        assert(scaled.getArea() == 200); // 10 x 20
        cout << "Scaled rectangle area: " << scaled.getArea() << endl;
